...
type Config struct {
	Region   string
	Endpoint string // optional, e.g. http://localhost:4566 (LocalStack) or http://127.0.0.1:9000 (MinIO)
}
...
func ConfigFromEnv(defRegion string) Config {
	...
	return Config{Region: region, Endpoint: strings.TrimSpace(os.Getenv("S3_ENDPOINT"))}
}
